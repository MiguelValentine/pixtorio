...
The project has a broad Aseprite-inspired raster, animation, interchange, history, compositing and MCP baseline. This document is the authoritative feature inventory and verification checkpoint; implemented workflows do not imply complete Aseprite parity. The explicit exclusions at the end of this document remain excluded.
...
The feature checkpoint records the following implemented workflows in addition to the baseline above:

- Drawing wraps writes across tiled document boundaries. Symmetry axes, grid origins and guides support direct dragging as well as numeric controls.
- Independent Move Tool supports active-Cel dragging, Ctrl/Cmd topmost-Cel auto-selection, persistent auto-select, Shift axis locking and multi-Cel movement, with one history command per gesture. Zoom supports cursor-anchored left-click zoom in and right-click zoom out. Polyline and Curve use staged points, live previews and Enter/double-click/right-click completion; Blur and Jumble use bounded edits.
- Background layers are unique, root-bottom, normal-blend, fully opaque and alpha-locked, with protected movement/merge behavior and ordinary-role duplicates. Erasing, deleting, cutting, moving or transforming background pixels writes the configured opaque background color, including linked/indexed Cel and clipboard paths.
- Edit commands include Copy Merged, Fill Selection, Stroke Selection, selection-bounded wrapped Shift Pixels, Paste Special to a new sprite/new layer/reference layer, and copy/cut selection to a new layer, with sparse/indexed data and reversible history.
- Cel opacity/z-index properties support batch and mixed-value editing, independent linked-Cel properties, clipboard/duplication, frame/region/export/onion compositing, Go thumbnails and MCP metadata/edits. Background Cels remain bottommost with fixed properties.
- Pattern brushes capture selection RGBA textures and bitmap footprints. Transparent texels are no-ops; texture alpha composes with dynamics, tiled writes retain phase, indexed/grayscale colors remain constrained and selection clipping/index synchronization are preserved. Strict versioned local presets support save, update, apply and delete of the complete brush/pattern/dynamics configuration.
- Brush size, opacity, angle and foreground/background color dynamics are independent channels with enable state, Pressure/Velocity source, min/max, threshold, inversion and linear/ease-in/ease-out/S-curve response. Disabled channels use ordinary brush values; shape and paint groups share live-stroke and strict v2 preset behavior.
- Text has live preview, foreground fill, background-color stroke and width, runtime font loading and one-command placement history. Ligature-disabled runs are measured/rendered per character; none/slight/full hinting uses unrestricted/half-pixel/integer placement. Loaded fonts become pixels rather than project dependencies.
- Outline FX supports inside/outside placement, thickness, square/diamond/circle shapes, eight directions, channel masks, X/Y tiled sampling, editable-Cel scope and isolated before/after preview. Partial Cels expand in document coordinates while retaining off-canvas pixels and linked relative offsets; oversized batches reject before mutation and feathered edges use premultiplied interpolation.
- Color Curves provide independent multi-point R/G/B/A curves, piecewise-linear LUTs, pointer/keyboard/numeric editing, protected endpoints, interior-point removal and per-channel reset, sharing Cel scope, channel masks, selection clipping and history.
- Indexed-color management supports appearance-preserving transparent-slot relocation and local default palettes with alpha/transparent index. Save/apply/reset workflows feed new-document defaults; applying is one reversible command. Invalid local defaults fall back to the built-in palette without repairing project data. Linked buffers, shorter-palette remapping, indexed tilesets and tilemap caches remain synchronized.
